...
export default function DropdownMessage(props) {
    ...
    const messagescount1 = datamessages.filter(
        (messages) => messages.read === false
    ).length;
    ...
    return (
        <Menu>
            ...
            >
                <Menu.Items
                    as="div"
                    className="w-72 transform translate-x-28 lg:translate-x-0 origin-top-right absolute ltr:right-0 rtl:left-0 rounded top-full z-50 py-0.5 ltr:text-left rtl:text-right bg-white dark:bg-gray-800 border dark:border-gray-700 shadow-md"
                >
                    {/* Head messages */}
                    ...
                    <div className="max-h-60 overflow-y-auto scrollbars show">
                        {messagescount > 0 ? (
                            datamessages
                                .filter((messages) => messages.read === false)
                                .map((filteredmessages) => (
                                    <Link
                                        key={filteredmessages.id}
                                        to={filteredmessages.url}
                                        className="relative"
                                    >
                    ...
                    <div className="p-3 text-center font-normal">
                        <Link
                            to={text_msg.footer_url}
                            className="hover:underline"
                        >
                            {text_msg.footer_text}
                        </Link>
                    </div>
                </Menu.Items>
            </Transition>
